...
The archive-root `VERSION` file contains the build identifier printed by `logs.ks` at boot as `CODE version=...`. Boot copies it into `1:/state/code_version.state` when connected, so archived logs can be matched back to the exact code loaded by the vessel even after later changes. Published GitHub releases attach a generated release bundle with a fresh UUID written to `VERSION`, plus the standalone `VERSION` file. Use `make release-version TAG=kos-YYYYMMDD-N` when you want a named Git tag for a release.
