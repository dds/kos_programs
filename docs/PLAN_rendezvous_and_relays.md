...
- `missions/FR3C/minmus_relay_tripack.ks` — full sequence to 500 km
  equatorial, 3 relays.
...
1. Vessel name collision — parked pod and Jeb's ship both default to "Falcon";
   ...
   vessels.
2. No Falcon rescue profile for the Ellory leg (only `rescue_lko.ks` on
   FR3/FR3b exists).
...
- `missions/Falcon/launch_to_rendezvous.ks` → `RENDEZVOUS_TARGET = Grumpy Bear`.
- `missions/Falcon/rescue_ellory.ks` — descent tags wired for the real
...
### A4 — Rescue Ellory from "Ellory's Wreckage" (`rescue_ellory.ks`, new)
- Profile mirrors `missions/FR3b/rescue_lko.ks`, Falcon-flavored:
  ...
  - Landing keys block copied from `rescue_lko.ks`.
...
3. Reserved names: no bare `r`/`v`/`q`; no shadowing `up`/`north`/`body`/
   `target`/`alt`/`eta`.
4. After editing `lib/dependencies.ks`: confirm phase roots, bands, and the
   binding waterfall stay in sync.
...
6. New profile sanity: `rescue_ellory.ks` keys match keys consumed by
...
- Diff vs. this plan: A2 profile points at renamed target; `rescue_ellory.ks`
  mirrors `rescue_lko.ks` with Falcon-correct decoupler/landing keys.
...
- Confirm dependency edits are committed AND pushed (archive sync needs the push).
